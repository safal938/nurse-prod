...
import { ClinicalSession } from '../services/realtimewebsocket';
...
export const RealtimePage: React.FC<{ patient: Patient; onBack: () => void }> = ({ patient, onBack }) => {
          ...
          <>
            <div className="flex items-center gap-2 mr-2">
              ...
              <button
                onClick={() => {
                  if (sessionRef.current) {
                    sessionRef.current.finishConsultation();
                  }
                }}
                className="ml-1 w-6 h-6 rounded flex items-center justify-center bg-emerald-500/80 hover:bg-emerald-600 transition-colors"
                title="End Consultation & Generate Report"
              >
            ...
            checklistItems.length === 0 ? (
              <div className="h-full flex items-center justify-center">
                <div className="bg-white rounded-xl border border-neutral-200 p-12 text-center max-w-md">
                  ...
                  {hasStarted && (
                    <>
                    <p className='text-sm text-neutral-500 mb-2'> Done with the consultation? </p>
                    <button
                      onClick={() => sessionRef.current?.finishConsultation()}
                      className="px-6 py-2.5 bg-white border border-emerald-200 text-emerald-700 font-semibold text-sm rounded-lg hover:bg-emerald-50 transition-colors shadow-sm flex items-center gap-2 mx-auto group"
                    >
